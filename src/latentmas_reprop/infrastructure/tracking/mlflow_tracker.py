...
class MLflowTracker(ExperimentTrackerPort):
    ...
    def __init__(
        ...
        if tracking_uri is not None:
            ...
            self.is_remote = not tracking_uri.startswith("sqlite") and not tracking_uri.startswith("file")
        elif env_uri is not None and env_uri.strip():
            self.tracking_uri = env_uri.strip()
            self.is_remote = not self.tracking_uri.startswith("sqlite") and not self.tracking_uri.startswith("file")
        else:
        ...
        if not self.is_remote:
            self.artifact_location = (
                artifact_location
                or str((self.resolver.cache_dir / "mlflow_artifacts").resolve())
            )
